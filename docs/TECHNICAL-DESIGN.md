...
- **Moves as authored compositions:** **Templates** (e.g. **Blast**, **Slam**) expose numeric bands + optional affinity slots; **surges** add **`delivery_modalities`** (concussive / piercing / slashing ω) like strikes; **`cooldown_scaling`** ties recharge wait to **`base_power`**. Players bind sliders at resolve time — including future **status**, **accumulator impulse**, and **passive hook** slots aligned with creature battle state ([`COMBAT-MODEL.md`](./COMBAT-MODEL.md) §5.4b, §3 and [`GAMEPLAY-SYSTEMS.md`](./GAMEPLAY-SYSTEMS.md)).
